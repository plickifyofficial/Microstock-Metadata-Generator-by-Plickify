...
export function metadataToText(item: GenerationResult): string {
  return [
    `File: ${item.filename}`,
    `Title: ${item.title}`,
...
interface Props {
  item: GenerationResult & { previewUrl?: string };
  onUpdate: (patch: Partial<GenerationResult>) => void;
  onRegenerate: () => void;
  onRemove: () => void;
}
            ...
            <p className="text-xs">
              {item.status === "done" && (
                <span className="text-emerald-600 dark:text-emerald-400">
                  Done - {item.keywords.length} keywords
                </span>
              )}
        ...
        <div className="flex items-center gap-1 shrink-0">
          {!busy && (
            <>
              ...
              <IconBtn title="Remove" onClick={onRemove} disabled={busy}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="h-4 w-4">
...
function IconBtn({
  ...
  return (
    <button
      ...
      aria-label={title}
      disabled={disabled}
      className="rounded-lg p-1.5 text-slate-500 hover:text-brand hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors disabled:opacity-30"
    >
      {children}
    </button>
  );
